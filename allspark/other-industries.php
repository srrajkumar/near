<div class="row submenu-tiles perspective" style="margin: 0;">
    <h3>Other Industries</h3>
    <div class="space" style="width: 100%; padding: 5px 0;"></div>

    <a href="<?php echo SITE_URL; ?>allspark/retail.php" class="col-md-4" data-aos="3dflip" data-aos-delay="200">
        <img src="<?php IMAGE('allspark/retail.png') ?>" alt="" class="sq-ratio">
        <span class="tile__content">
            <h4>Retail</h4>
            <hr />
            <p class="description">
                Retailers can leverage Allspark to 
            </p>
        </span>
        <span class="custom_btn">
            <span class="btn__text">
                <span class="btn__text-inner">Explore</span>
            </span>            
            <div class="icon">
                <svg class="" viewBox="0 0 18 7" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14,3.09H0" fill="#fff"></path>
                    <polyline class="color_change" points="11.34 5.83 13.97 3.09 11.34 0.35" fill="#fff"></polyline>
                </svg>
            </div>
        </span>
   </a>


   <a href="<?php echo SITE_URL; ?>allspark/qsr.php" class="col-md-4" data-aos="3dflip" data-aos-delay="400">
        <img src="<?php IMAGE('allspark/qsr.png') ?>" alt="" class="sq-ratio">
        <span class="tile__content">
            <h4>QSR</h4>
            <hr />
            <p class="description">
              Allspark can be used by QSRs to 
            </p>
        </span>
        <span class="custom_btn">
            <span class="btn__text">
                <span class="btn__text-inner">Explore</span>
            </span>            
            <div class="icon">
                <svg class="" viewBox="0 0 18 7" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14,3.09H0" fill="#fff"></path>
                    <polyline class="color_change" points="11.34 5.83 13.97 3.09 11.34 0.35" fill="#fff"></polyline>
                </svg>
            </div>
        </span>
   </a>

    <a href="<?php echo SITE_URL; ?>allspark/telco.php" class="col-md-4" data-aos="3dflip" data-aos-delay="600">
        <img src="<?php IMAGE('allspark/telco.png') ?>" alt="" class="sq-ratio">
        <span class="tile__content">
            <h4>Telco</h4>
            <hr />
            <p class="description">
              Telecommunication players can use Allspark to
            </p>
        </span>
        <span class="custom_btn">
            <span class="btn__text">
                <span class="btn__text-inner">Explore</span>
            </span>            
            <div class="icon">
                <svg class="" viewBox="0 0 18 7" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14,3.09H0" fill="#fff"></path>
                    <polyline class="color_change" points="11.34 5.83 13.97 3.09 11.34 0.35" fill="#fff"></polyline>
                </svg>
            </div>
        </span>
   </a>

    <a href="<?php echo SITE_URL; ?>allspark/auto.php" class="col-md-4" data-aos="3dflip" data-aos-delay="800">
        <img src="<?php IMAGE('allspark/auto.png') ?>" alt="" class="sq-ratio">
        <span class="tile__content">
            <h4>Auto</h4>
            <hr />
            <p class="description">
               Allspark can be used by the Auto players to
            </p>
        </span>
        <span class="custom_btn">
            <span class="btn__text">
                <span class="btn__text-inner">Explore</span>
            </span>            
            <div class="icon">
                <svg class="" viewBox="0 0 18 7" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14,3.09H0" fill="#fff"></path>
                    <polyline class="color_change" points="11.34 5.83 13.97 3.09 11.34 0.35" fill="#fff"></polyline>
                </svg>
            </div>
        </span>
   </a>
    <a href="<?php echo SITE_URL; ?>allspark/bfsi.php" class="col-md-4" data-aos="3dflip" data-aos-delay="800">
        <img src="<?php IMAGE('allspark/bfsi.png') ?>" alt="" class="sq-ratio">
        <span class="tile__content">
            <h4>BFSI</h4>
            <hr />
            <p class="description">
               Allspark can be used by the BFSI industry
            </p>
        </span>
        <span class="custom_btn">
            <span class="btn__text">
                <span class="btn__text-inner">Explore</span>
            </span>            
            <div class="icon">
                <svg class="" viewBox="0 0 18 7" xmlns="http://www.w3.org/2000/svg">
                    <path d="M14,3.09H0" fill="#fff"></path>
                    <polyline class="color_change" points="11.34 5.83 13.97 3.09 11.34 0.35" fill="#fff"></polyline>
                </svg>
            </div>
        </span>
    </a>

</div>